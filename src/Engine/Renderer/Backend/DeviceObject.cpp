    ...
    void DeviceObject::CreateDebugInfo(const std::string& debugName,
        ...
        if (!debugName.empty()) {
            if (vkDebugMarkerSetObjectNameEXT != nullptr) {
                VkDebugMarkerObjectNameInfoEXT objectInfo{ VK_STRUCTURE_TYPE_DEBUG_MARKER_OBJECT_NAME_INFO_EXT };
            ...
            Logger::Trace("Initialised device object %s %s", typeid(*this).name(), debugName.c_str());
        } else {
